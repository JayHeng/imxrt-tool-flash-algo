                                   ...
                                   usdhc_transfer_t *transfer)
{
    ...
    if (handle->data != NULL)
    {
        USDHC_ClearInterruptStatusFlags(base,
                                        (enDMA == false ? kUSDHC_DataFlag : kUSDHC_DataDMAFlag) | kUSDHC_CommandFlag);
        USDHC_EnableInterruptSignal(base, (enDMA == false ? kUSDHC_DataFlag : kUSDHC_DataDMAFlag) | kUSDHC_CommandFlag);
    }
    else
    {
...
static void USDHC_TransferHandleData(USDHC_Type *base, usdhc_handle_t *handle, uint32_t interruptFlags)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (interruptFlags & kUSDHC_DataCompleteFlag)
            {
                transferStatus = kStatus_Success;
            }
        }
    ...
    if ((handle->callback.TransferComplete) && (transferStatus != kStatus_USDHC_BusyTransferring))
    {
        handle->callback.TransferComplete(base, handle, transferStatus, handle->userData);
        USDHC_DisableInterruptSignal(base, kUSDHC_DataFlag | kUSDHC_DataDMAFlag);
        handle->data = NULL;
    }
}
