...
{
  return (CLKCTL0->SYSOSCBYPASS == 0U) ? CLK_XTAL_OSC_CLK : ((CLKCTL0->SYSOSCBYPASS == 1U) ? CLK_CLK_IN : 0U);
}
...
__attribute__ ((weak)) void SystemInit (void) {
  ...
  {
      clkgate = CLKCTL0->PSCCTL0;
      /* Ensure FLEXSPI0 clock is enabled as CACHE64_CTRL0 depends on that.
       * It's only needed in debugging application running in RAM. If the application
       * is boot from ROM, the FLEXSPI0 clock has already been enabled. */
      CLKCTL0->PSCCTL0 = clkgate | CLKCTL0_PSCCTL0_FLEXSPI0_OTFAD_CLK_MASK;
      ...
      /* Restore clock gate setting */
      CLKCTL0->PSCCTL0 = clkgate;
  }
