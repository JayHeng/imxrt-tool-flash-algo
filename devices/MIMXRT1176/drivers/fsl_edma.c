...
void EDMA_TcdSetTransferConfig(edma_tcd_t *tcd, const edma_transfer_config_t *config, edma_tcd_t *nextTcd)
{
    ...
    assert(config != NULL);
    assert(((uint32_t)nextTcd & 0x1FU) == 0U);
    assert((config->srcAddr % (1U << (uint32_t)config->srcTransferSize)) == 0U);
    assert((config->destAddr % (1U << (uint32_t)config->destTransferSize)) == 0U);
                                ...
                                uint32_t transferBytes)
{
    ...
    assert(((uint32_t)srcAddr % srcWidth) == 0U);
    assert(((uint32_t)destAddr % destWidth) == 0U);
