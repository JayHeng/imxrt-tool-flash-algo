 ...
 */
static void CSI_TransferPutEmptyBuffer(csi_handle_t *handle, void *buffer);
...
static uint32_t CSI_TransferGetEmptyBuffer(csi_handle_t *handle)
{
    ...
    buffer = handle->emptyBuffer;
    handle->emptyBufferCnt--;
    handle->emptyBuffer = *(void **)buffer;
...
static void CSI_TransferPutEmptyBuffer(csi_handle_t *handle, void *buffer)
{
    *(void **)buffer    = handle->emptyBuffer;
    handle->emptyBuffer = buffer;
    handle->emptyBufferCnt++;
}
...
status_t CSI_Init(CSI_Type *base, const csi_config_t *config)
{
     ...
     */
    if ((kCSI_DataBus8Bit == config->dataBus) && (2 == config->bytesPerPixel))
    {
        busCyclePerPixel = 2;
    }
    else
    ...
    /* Image parameter. */
    base->CSIIMAG_PARA = ((uint32_t)(config->width * busCyclePerPixel) << CSI_CSIIMAG_PARA_IMAGE_WIDTH_SHIFT) |
...
status_t CSI_TransferStop(CSI_Type *base, csi_handle_t *handle)
{
    ...
    {
        CSI_TransferPutEmptyBuffer(handle, (void *)(CSI_GetRxBufferAddr(base, bufIdx)));
    }
...
status_t CSI_TransferSubmitEmptyBuffer(CSI_Type *base, csi_handle_t *handle, uint32_t frameBuffer)
{
    ...
    CSI_TransferPutEmptyBuffer(handle, (void *)frameBuffer);
...
status_t CSI_TransferGetFullBuffer(CSI_Type *base, csi_handle_t *handle, uint32_t *frameBuffer)
{
    ...
    if (handle->queueReadIdx == handle->queueWriteIdx)
    {
...
void CSI_TransferHandleIRQ(CSI_Type *base, csi_handle_t *handle)
{
    ...
    else if (csisr & (CSI_CSISR_DMA_TSF_DONE_FB2_MASK | CSI_CSISR_DMA_TSF_DONE_FB1_MASK))
    {
        ...
        if (handle->activeBufferNum < CSI_MAX_ACTIVE_FRAME_NUM)
        {
            if (CSI_TransferGetEmptyBufferCount(handle) > 0)
            {
            ...
            else
            {
                 ...
                 */
                frameBuffer = CSI_GetRxBufferAddr(base, dmaDoneBufferIdx ^ 1);
