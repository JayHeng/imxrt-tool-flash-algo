               ...
               uint32_t srcClock_Hz)
{
    ...
    assert(bufferConfig);
    assert(macAddr);
    assert(config->ringNum <= FSL_FEATURE_ENET_INSTANCE_QUEUEn(base));
    ...
    for (uint8_t count = 0; count < handle->ringNum; count++)
    {
                            ...
                            const enet_buffer_config_t *bufferConfig)
{
    ...
    handle->ringNum = (config->ringNum > FSL_FEATURE_ENET_INSTANCE_QUEUEn(base)) ?
                          FSL_FEATURE_ENET_INSTANCE_QUEUEn(base) :
                          config->ringNum;
...
#if FSL_FEATURE_ENET_QUEUE > 1
        for (queue = 0; queue < FSL_FEATURE_ENET_INSTANCE_QUEUEn(base); queue++)
        {
...
static void ENET_ActiveSend(ENET_Type *base, uint32_t ringId)
{
    assert(ringId < FSL_FEATURE_ENET_INSTANCE_QUEUEn(base));
...
void ENET_AVBConfigure(ENET_Type *base, enet_handle_t *handle, const enet_avb_config_t *config)
{
    ...
    for (count = 0; count < FSL_FEATURE_ENET_INSTANCE_QUEUEn(base) - 1; count++)
    {
