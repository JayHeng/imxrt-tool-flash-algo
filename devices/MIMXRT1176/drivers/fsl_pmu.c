...
#define PMU_LDO_ENABLE_SETPOINT_REGISTERS                                                                          \
    {                                                                                                              \
        ANADIG_PMU->LDO_PLL_ENABLE_SP, ANADIG_PMU->LDO_LPSR_ANA_ENABLE_SP, ANADIG_PMU->LDO_LPSR_DIG_ENABLE_SP, 0UL \
    }

#define PMU_LDO_LP_MODE_EN_SETPOINT_REGISTERS                                              \
    {                                                                                      \
        0UL, ANADIG_PMU->LDO_LPSR_ANA_LP_MODE_SP, ANADIG_PMU->LDO_LPSR_DIG_LP_MODE_SP, 0UL \
    }

#define PMU_LDO_TRACKING_EN_SETPOINT_REGISTERS                                                     \
    {                                                                                              \
        0UL, ANADIG_PMU->LDO_LPSR_ANA_TRACKING_EN_SP, ANADIG_PMU->LDO_LPSR_DIG_TRACKING_EN_SP, 0UL \
    }

#define PMU_LDO_BYPASS_EN_SETPOINT_REGISTERS                                                   \
    {                                                                                          \
        0UL, ANADIG_PMU->LDO_LPSR_ANA_BYPASS_EN_SP, ANADIG_PMU->LDO_LPSR_DIG_BYPASS_EN_SP, 0UL \
    }

#define PMU_LDO_STBY_EN_REGISTERS                                                                                     \
    {                                                                                                                 \
        ANADIG_PMU->PLL_LDO_STBY_EN_SP, ANADIG_PMU->LDO_LPSR_ANA_STBY_EN_SP, ANADIG_PMU->LDO_LPSR_DIG_STBY_EN_SP, 0UL \
    }

#define PMU_LPSR_DIG_TRG_REGISTERS                                                                            \
    {                                                                                                         \
        ANADIG_PMU->LDO_LPSR_DIG_TRG_SP0, ANADIG_PMU->LDO_LPSR_DIG_TRG_SP1, ANADIG_PMU->LDO_LPSR_DIG_TRG_SP2, \
            ANADIG_PMU->LDO_LPSR_DIG_TRG_SP3                                                                  \
    }

#define PMU_BODY_BIAS_ENABLE_REGISTERS                                                              \
    {                                                                                               \
        ANADIG_PMU->FBB_M7_ENABLE_SP, ANADIG_PMU->RBB_SOC_ENABLE_SP, ANADIG_PMU->RBB_LPSR_ENABLE_SP \
    }

#define PMU_BODY_BIAS_STBY_EN_REGISTERS                                                                \
    {                                                                                                  \
        ANADIG_PMU->FBB_M7_STBY_EN_SP, ANADIG_PMU->RBB_SOC_STBY_EN_SP, ANADIG_PMU->RBB_LPSR_STBY_EN_SP \
    }

#define PMU_BODY_BIAS_CONFIGURE_REGISTERS                                                           \
    {                                                                                               \
        ANADIG_PMU->FBB_M7_CONFIGURE, ANADIG_PMU->RBB_SOC_CONFIGURE, ANADIG_PMU->RBB_LPSR_CONFIGURE \
    }
...
void PMU_GPCSetLpsrDigLdoTargetVoltage(uint32_t setpointMap, pmu_lpsr_dig_target_output_voltage_t voltageValue)
{
    ...
    for (regIndex = 0U; regIndex < ARRAY_SIZE(lpsrDigTrgRegArray); regIndex++)
    {
        ...
        if (temp8 != 0UL)
        {
            ...
            lpsrDigTrgRegArray[regIndex] = regValue;
        }
    }
}
...
void PMU_GPCEnableLdo(pmu_ldo_name_t name, uint32_t setpointMap, bool enable)
{
    ...
    if (enable)
    {
        ldoEnableRegArray[(uint8_t)name] &= ~setpointMap;
    }
    else
    {
        ldoEnableRegArray[(uint8_t)name] |= setpointMap;
...
void PMU_GPCSetLdoOperateMode(pmu_ldo_name_t name, uint32_t setpointMap, pmu_ldo_operate_mode_t mode)
{
    ...
    {
        ldoLpModeRegArray[(uint8_t)name] &= ~setpointMap;
    }
    else
    {
        ldoLpModeRegArray[(uint8_t)name] |= setpointMap;
...
void PMU_GPCEnableLdoTrackingMode(pmu_ldo_name_t name, uint32_t setpointMap, bool enable)
{
    ...
    if (enable)
    {
        ldoTrackingEnableRegArray[(uint8_t)name] |= setpointMap;
    }
    else
    {
        ldoTrackingEnableRegArray[(uint8_t)name] &= ~setpointMap;
...
void PMU_GPCEnableLdoBypassMode(pmu_ldo_name_t name, uint32_t setpointMap, bool enable)
{
    ...
    if (enable)
    {
        ldoBypassEnableRegArray[(uint8_t)name] |= setpointMap;
    }
    else
    {
        ldoBypassEnableRegArray[(uint8_t)name] &= ~setpointMap;
...
void PMU_GPCEnableLdoStandbyMode(pmu_ldo_name_t name, uint32_t setpointMap, bool enable)
{
    ...
    if (enable)
    {
        ldoStandbyEnableRegArray[(uint8_t)name] |= setpointMap;
    }
    else
    {
        ldoStandbyEnableRegArray[(uint8_t)name] &= ~setpointMap;
...
void PMU_StaticCm7FBBInit(ANADIG_PMU_Type *base, const pmu_static_body_bias_config_t *config)
{
    ...
    base->PMU_BIAS_CTRL &= ~ANADIG_PMU_PMU_BIAS_CTRL2_WB_PWR_SW_EN_1P8_MASK;
    base->PMU_BIAS_CTRL |= ANADIG_PMU_PMU_BIAS_CTRL2_WB_PWR_SW_EN_1P8(0x1U);
    base->PMU_BIAS_CTRL |= ANADIG_PMU_PMU_BIAS_CTRL2_WB_EN_MASK;
...
void PMU_StaticLpsrRBBInit(ANADIG_PMU_Type *base, const pmu_static_body_bias_config_t *config)
{
    ...
    base->PMU_BIAS_CTRL = temp32;
    base->PMU_BIAS_CTRL &= ~ANADIG_PMU_PMU_BIAS_CTRL2_WB_PWR_SW_EN_1P8_MASK;
    base->PMU_BIAS_CTRL |= ANADIG_PMU_PMU_BIAS_CTRL2_WB_PWR_SW_EN_1P8(0x2U);
    base->PMU_BIAS_CTRL |= ANADIG_PMU_PMU_BIAS_CTRL2_WB_EN_MASK;
    SDK_DelayAtLeastUs(100, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
}
...
void PMU_StaticSocRBBInit(ANADIG_PMU_Type *base, const pmu_static_body_bias_config_t *config)
{
    ...
    base->PMU_BIAS_CTRL = temp32;
    base->PMU_BIAS_CTRL &= ~ANADIG_PMU_PMU_BIAS_CTRL2_WB_PWR_SW_EN_1P8_MASK;
    base->PMU_BIAS_CTRL |= ANADIG_PMU_PMU_BIAS_CTRL2_WB_PWR_SW_EN_1P8(0x4U);
    base->PMU_BIAS_CTRL |= ANADIG_PMU_PMU_BIAS_CTRL2_WB_EN_MASK;
    SDK_DelayAtLeastUs(100, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
}
...
void PMU_GPCEnableBodyBias(pmu_body_bias_name_t name, uint32_t setpointMap, bool enable)
{
    ...
    if (enable)
    {
        bodyBiasEnableRegArray[(uint8_t)name] &= ~setpointMap;
    }
    else
    {
        bodyBiasEnableRegArray[(uint8_t)name] |= setpointMap;
...
void PMU_GPCEnableBodyBiasStandbyMode(pmu_body_bias_name_t name, uint32_t setpointMap, bool enable)
{
    ...
    if (enable)
    {
        BBStandbyEnableRegArray[(uint8_t)name] |= setpointMap;
    }
    else
    {
        BBStandbyEnableRegArray[(uint8_t)name] &= ~setpointMap;
...
void PMU_GPCSetBodyBiasConfig(pmu_body_bias_name_t name, const pmu_gpc_body_bias_config_t *config)
{
    ...
    temp32 = bodyBiasConfigRegArray[(uint8_t)name];
    ...
    bodyBiasConfigRegArray[(uint8_t)name] = temp32;
}
