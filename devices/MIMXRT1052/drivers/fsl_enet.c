...
void ENET_SetSMI(ENET_Type *base, uint32_t srcClock_Hz, bool isPreambleDisabled)
{
    ...
    speed = srcClock_Hz / (2U * ENET_MDC_FREQUENCY) - 1;
...
status_t ENET_GetRxFrameSize(enet_handle_t *handle, uint32_t *length, uint8_t ringId)
{
    ...
    *length = 0;
    ...
    if ((0U != (curBuffDescrip->control & ENET_BUFFDESCRIPTOR_RX_EMPTY_MASK)) ||
        (0U != (curBuffDescrip->control & ENET_BUFFDESCRIPTOR_RX_SOFTOWNER1_MASK)))
    {
        isReturn = true;
        ...
        else
        {
            ...
            else
            {
                ...
                do
                {
                    ...
                    {
                        /* Data copy. */
                        (void)memcpy((void *)address, (void *)src, handle->txBuffSizeAlign[ringId]);
#if defined(FSL_SDK_ENABLE_DRIVER_CACHE_CONTROL) && FSL_SDK_ENABLE_DRIVER_CACHE_CONTROL
                        if (handle->txMaintainEnable[ringId])
                        {
 ...
 * param ringId The ring index or ring number.
 * retval kStatus_Success  Set Tx Reclaim succeed.
...
static void ENET_ReclaimTxDescriptor(ENET_Type *base, enet_handle_t *handle, uint8_t ringId)
{
    ...
    while ((txBdRing->txDescUsed > 0) && (0U == (curBuffDescrip->control & ENET_BUFFDESCRIPTOR_TX_READY_MASK)))
    {
...
#ifdef ENET_ENHANCEDBUFFERDESCRIPTOR_MODE
            if ((curBuffDescrip->controlExtend1 & ENET_BUFFDESCRIPTOR_TX_TIMESTAMP_MASK) && (txDirty != NULL))
            {
...
void ENET_ReleaseRxBuffer(ENET_Type *base, enet_handle_t *handle, void *buffer, uint8_t ringId)
{
    ...
    do
    {
        ...
        if ((buffer == ownBuffDescrip->buffer) &&
            (0U != (ownBuffDescrip->control & ENET_BUFFDESCRIPTOR_RX_SOFTOWNER1_MASK)))
        {
            isReleaseBd = true;
            break;
        }
...
#endif /* FSL_FEATURE_MEMORY_HAS_ADDRESS_OFFSET */
                curBuffDescrip->buffer = (uint8_t *)data;
                /* Set data length. */
                curBuffDescrip->length = (uint16_t)length;
#ifdef ENET_ENHANCEDBUFFERDESCRIPTOR_MODE
                /* For enable the timestamp. */
                          ...
                          void *context)
{
    ...
    else
    {
        ...
        else
        {
...
#ifdef ENET_ENHANCEDBUFFERDESCRIPTOR_MODE
            /* For enable the timestamp. */
            if ((txFlag & ENET_TX_TIMESTAMP_FLAG) != 0)
            {
                curBuffDescrip->controlExtend1 |= ENET_BUFFDESCRIPTOR_TX_TIMESTAMP_MASK;
            }
            else
            ...
            if ((txFlag & ENET_TX_LAST_BD_FLAG) != 0)
            {
                ...
                do
                {
                    ...
                    {
                        tempBuffDescrip->control &= ~ENET_BUFFDESCRIPTOR_TX_SOFTOWENER1_MASK;
...
/* Check if the transmit interrupt happen. */
    ...
    while (0U != (mask & base->EIR))
    {
        ...
        /* Callback Handler. */
        if (handle->TxReclaimEnable[index] && (irq & (uint32_t)kENET_TxFrameInterrupt))
        {
...
void ENET_CommonFrame0IRQHandler(ENET_Type *base)
{
    ...
    if (0U != (event & ENET_TS_INTERRUPT) && s_enetTsIsr[instance])
    {
        s_enetTsIsr[instance](base, s_ENETHandle[instance]);
    }
    if (0U != (event & ENET_ERR_INTERRUPT) && s_enetErrIsr[instance])
    {
